...
# This is a "standard" set of maximum verse numbers by book and chapter that can be
# overridden by specific Bible versions if necessary.
...
@lru_cache()
def get_number_of_chapters(book: Book, bible: Bible | None = None) -> int:
...
@lru_cache()
def is_single_chapter_book(book: Book, bible: Bible | None = None) -> bool:
    ...
    :param bible: an optional Bible object to check for chapter existence
    :type bible: Bible | None
    :return: True if the given book is a single chapter book; otherwise, False
    :rtype: bool
    """
    return get_number_of_chapters(book, bible) == 1


@lru_cache()
def get_number_of_verses(book: Book, chapter: int, bible: Bible | None = None) -> int:
    ...
    chapter_list: list[int] = MAX_VERSE_NUMBER_BY_BOOK_AND_CHAPTER.get(book, [])
...
def get_verse_id(
    ...
    :raises InvalidVerseError: if the verse doesn't exist for the given book, chapter,
                               and verse
    """
    max_verse_number: int | None = get_number_of_verses(book, chapter, bible)
