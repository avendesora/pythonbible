...
def test_get_references(text_with_reference):
    ...
    references = pybible.get_references(text_with_reference)

    # Then the references are found and returned in a list of normalized reference tuples
    assert len(references) == 2
    assert references[0] == (pybible.Book.MATTHEW, 18, 12, 18, 14)
    assert references[1] == (pybible.Book.LUKE, 15, 3, 15, 7)
...
def test_get_references_complex(
    ...
    # When parsing that text
    references = pybible.get_references(text_with_reference_complex)
...
def test_normalize_reference(non_normalized_reference):
    ...
    normalized_references = pybible.normalize_reference(non_normalized_reference)
    ...
    assert normalized_references[0] == (pybible.Book.MATTHEW, 18, 12, 18, 14)
...
def test_normalize_reference_without_verse_numbers(reference_without_verse_numbers):
    ...
    # When we normalize that reference
    normalized_references = pybible.normalize_reference(reference_without_verse_numbers)

    # Then the resulting normalized references contain the proper verse numbers
    assert len(normalized_references) == 1
    assert normalized_references[0] == (pybible.Book.GENESIS, 1, 1, 4, 26)
